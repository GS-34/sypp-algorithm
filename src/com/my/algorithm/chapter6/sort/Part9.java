...
public class Part9 {
    ...
    public static void main(String[] args) {
        ...
        int n = kb.nextInt(); //노래의 갯수
        int k = kb.nextInt(); //DVD 갯수

        int[] capacities = new int[n];

        int total = 0;//모든 용량을 합한 값
        int max = 0;// 메모리중 가장 큰 값을 저장한다.(최소 DVD 한장엔 들어가도록)

        //max ~ total 의 용량으로 간추려진다.
        ...
        while (l <= r){
            int curCapacity = (l + r) / 2;
            int sum = 0;
            int dvdCount = 1;
            for(int c : capacities){

                if(curCapacity < sum + c){
                    dvdCount++;
                    if(dvdCount > k) break;
                    sum = c;
                } else {
