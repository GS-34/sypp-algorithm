  ...
  public static String solution(String new_id) {
    ...
    for(int i = 0; i < chars.length; i++){
      ...
      if(('0' <= c && c <= '9') || ('a' <= c && c <= 'z') || c == '-' || c == '_'|| c == '.') {
        ...
        if(!queue.isEmpty() && queue.peekLast() == '.' && c == '.') continue;
    ...
    if (queue.isEmpty()) queue.offer('a');


    if (queue.size() >= 16) {
      Deque<Character> tmpQueue = new ArrayDeque<>();
      while (tmpQueue.size() < 15){
    ...
    if(queue.size() <= 2) {
      while (queue.size() < 3){
        queue.offer(queue.getLast().charValue());
      }
    }

    StringBuilder sb = new StringBuilder();
    queue.stream().forEach(c -> sb.append(c));
